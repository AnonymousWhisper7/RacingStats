...
function TrackMapPanel({ track, live }: { track: TrackPayload | null; live: LiveTelemetrySnapshot | null }) {
  ...
  const points = polyline.map((point) => `${point.x * 1000},${point.y * 700}`).join(" ");
  const dotX = (live?.x ?? polyline[0]?.x ?? 0.5) * 1000;
  const dotY = (live?.y ?? polyline[0]?.y ?? 0.5) * 700;
    ...
    <section style={{ ...panelStyle, padding: 22 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <h2 style={{ margin: "8px 0 6px", fontSize: 28 }}>{live?.track_name ?? track?.track.display_name ?? "Imola"}</h2>
      ...
      <div style={{ background: "linear-gradient(180deg, rgba(59,130,246,0.08), rgba(2,6,23,0.1))", borderRadius: 24, overflow: "hidden" }}>
        <svg viewBox="0 0 1000 700" style={{ width: "100%", height: 520, display: "block" }}>
          {[...Array(12)].map((_, idx) => (
            <line key={`v-${idx}`} x1={idx * 90 + 5} y1={0} x2={idx * 90 + 5} y2={700} stroke="rgba(148,163,184,0.08)" />
          ))}
          {[...Array(8)].map((_, idx) => (
            <line key={`h-${idx}`} x1={0} y1={idx * 90 + 8} x2={1000} y2={idx * 90 + 8} stroke="rgba(148,163,184,0.08)" />
          ))}
          ...
          {(track?.reference_line.brake_markers ?? []).map((marker) => {
            ...
            return (
              <g key={marker.corner_id}>
                <circle cx={corner.x * 1000} cy={corner.y * 700} r={16} fill="rgba(239,68,68,0.18)" stroke="rgba(248,113,113,0.75)" strokeWidth={2} />
                <text x={corner.x * 1000 + 24} y={corner.y * 700 + 6} fill="#fecaca" fontSize="16" fontFamily="Inter, Arial, sans-serif">
                  Brake
                </text>
              </g>
            );
          ...
          {corners.map((corner) => (
            <g key={corner.id}>
              <circle
                ...
                strokeWidth={2}
              />
              <text x={corner.x * 1000 + 16} y={corner.y * 700 - 14} fill="#cbd5e1" fontSize="16" fontFamily="Inter, Arial, sans-serif">
                {corner.name}
              </text>
            </g>
          ))}
